...
if (!email && !uid) {
  console.error('Usage: npm run set-admin -- --email=user@example.com [--role=admin|manager|staff] [--serviceAccount=path]')
  console.error('   or: npm run set-admin -- --uid=UID [--role=admin|manager|staff] [--serviceAccount=path]')
